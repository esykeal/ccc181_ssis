...
interface EditProgramDialogProps {
  ...
  program: Program | null; // The program to edit
...
export default function EditProgramDialog({
  ...
  // 1. Fetch Colleges List (for the dropdown options)
  useEffect(() => {
    api
      .get("/colleges/")
  ...
  // 2. Sync Form with Program Data (Pre-fill)
  useEffect(() => {
    if (program) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 3. Send PUT Request
      // URL: Use the ORIGINAL code (program.program_code) to identify the record
      // Body: Use the NEW state (code, name, selectedCollege)
