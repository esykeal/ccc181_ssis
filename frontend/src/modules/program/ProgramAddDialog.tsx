...
export default function AddProgramDialog({
  ...
  // Data State
  const [colleges, setColleges] = useState<College[]>([]); // For the dropdown options
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  // 1. Fetch Colleges when the dialog loads (so we can show them in the list)
  useEffect(() => {
    api
      .get("/colleges/")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 2. Send Data (Linking program to the selected college)
      ...
      setName("");
      setSelectedCollege("");
      onProgramAdded(); // Refresh the main list
    } catch (err: any) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          {/* College Dropdown (The Relationship) */}
          <div className="grid grid-cols-4 items-center gap-4">
